...
export default class Level6Scene extends BaseLevelScene {
  ...
  create() {
    this.createLevel({
      levelNumber: 6,
      title: 'Nivel 6 — Catacumbas ardientes',
      mapKey: 'nivel6',
    ...
    this.createDangerZoneDamage(LAVA_DAMAGE, LAVA_DAMAGE_COOLDOWN, 'Lava cercana: pierdes HP si te quedas ahi');
    this.createMovingPlatforms();
    this.startTimedWaves();
    this.showObjective('Objetivo: sobrevive a 3 oleadas, evita la lava y entra al portal.', { backgroundColor: 'rgba(20, 10, 8, 0.82)' });
  }
  ...
  checkLevelCompletion() {
    ...
    if (wavesDone && activeEnemies === 0 && !this.portal) {
      addScore(this, 100);
      this.showObjective('¡Catacumbas completadas! Entra al portal para elegir habilidad y visitar la tienda.', { backgroundColor: 'rgba(20, 10, 8, 0.82)' });
      this.createPortal(() => this.completeLevel(() => {
        this.scene.start('AbilityScene', {
          levelCompleted: 6,
          nextScene: 'TiendaScene',
          nextLevelScene: 'Level7Scene'
        });
      }), this.portalPosition);
    }
  }
